...
const seedUsers = async () => {
  try {
    ...
    const registeredUsers = await User.find({}).select("-password -refreshToken") ;
    ...
    const friendship = await Friendship.create([
      {
        ...
        status: "accepted",
        blockedBy: registeredUsers[1]._id,
      },
      {
      ...
      {
        requester: registeredUsers[0]._id,
        recipient: registeredUsers[4]._id,
        status: "accepted",
        blockedBy: registeredUsers[4]._id,
      },
      {
        requester: registeredUsers[5]._id,
        recipient: registeredUsers[0]._id,
        status: "accepted",
        blockedBy: registeredUsers[5]._id,
      },
      {
        requester: registeredUsers[4]._id,
        recipient: registeredUsers[6]._id,
        status: "blocked",
        blockedBy: registeredUsers[4]._id,
      }
    ]);
    console.log("Friendship seeded successfully:", friendship.map(f => ({ id: f._id, status: f.status })));
  } catch (error) {
